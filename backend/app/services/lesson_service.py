...
"""
    ...
    def update_lesson_status(self, user: User, lesson_id: int, new_status: str) -> bool:
        ...
        # Check ownership
        plan_orm = self.db.scalar(select(LearningPlanORM).where(LearningPlanORM.id == lesson.plan_id).where(LearningPlanORM.user_id == user.id))
        if not plan_orm:
...
"""
    ...
    def submit_test(self, user: User, lesson_id: int, submission: TestSubmissionRequest) -> Optional[TestSubmissionResponse]:
        if user.id is None:
            raise HTTPException(status_code=401, detail="User ID not found")
