...
class TestAgentRefactor(unittest.TestCase):
    def test_recommendation_validation(self):
        ...
        rec = AgentRecommendation(
            score=0.9, reasoning="This is a very good course for you.", tags=["Highly Recommended"]
        )
        self.assertEqual(rec.score, 0.9)

        # Invalid: High score with short reasoning
        with self.assertRaises(ValidationError):
            AgentRecommendation(score=0.9, reasoning="Short.", tags=["Tag"])

        # Low score with short reasoning should be fine (if not explicitly blocked)
        rec_low = AgentRecommendation(score=0.1, reasoning="Not fit.", tags=["Irrelevant"])
    ...
    def test_parse_agent_recommendation(self):
        ...
        text = (
            "Based on my analysis, here is the result: ```json\n"
            + json.dumps(data)
