...
from app.infrastructure.db.models import UserORM, LessonORM, LearningPlanORM, UserTestScoreORM
...
def test_save_test_score(db: Session):
    ...
    lesson = LessonORM(plan_id=plan.id, order=1, title="Test Lesson", description="Test Description")
    db.add(lesson)
    db.commit()
