...
export function OnboardingWizard() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden transition-all border border-slate-200">
        {/* Progress Bar */}
        ...
        <div className="p-8">
          {step === 1 && (
            <div className="space-y-6 text-slate-900">
              <div className="space-y-2">
                <h2 className="text-2xl font-bold text-slate-900">Welcome to EduPath AI!</h2>
                <p className="text-slate-500">Let&apos;s start by getting to know you better.</p>
              </div>
          ...
          {step === 2 && (
            <div className="space-y-6 text-slate-900">
              <div className="space-y-2">
                <h2 className="text-2xl font-bold text-slate-900">What are you interested in?</h2>
                <p className="text-slate-500">Add tags for skills or topics you want to learn.</p>
              </div>
              ...
              <div className="space-y-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="block w-full pl-10 pr-4 py-3 bg-white text-slate-900 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none"
                ...
                <div className="flex flex-wrap gap-2 min-h-[100px] p-4 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                  {interests.length === 0 ? (
                    ...
                    interests.map((tag) => (
                      <span
                        key={tag}
                        className="flex items-center gap-1 px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm font-medium border border-blue-200"
                      >
                        {tag}
                        <button onClick={() => removeInterest(tag)} className="hover:text-blue-900">
                          <X className="w-3 h-3" />
                        </button>
                      </span>
                    ))
              ...
              <div className="flex gap-4">
                <button
                  onClick={prevStep}
                  className="flex-1 flex items-center justify-center gap-2 border border-slate-200 hover:bg-slate-50 text-slate-600 font-semibold py-3 rounded-xl transition-all"
                >
                ...
                <button
                  onClick={nextStep}
                  className="flex-[2] flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 rounded-xl transition-all shadow-lg shadow-blue-200"
                >
                  Next Step
                  <ArrowRight className="w-5 h-5" />
                </button>
            ...
            <div className="space-y-6 text-center text-slate-900">
              ...
              <div className="bg-slate-50 p-6 rounded-2xl text-left space-y-3 border border-slate-100">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-semibold text-slate-900">{getValues().full_name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">Goal</span>
                  <span className="text-sm font-semibold truncate max-w-[200px] text-slate-900">{getValues().career_goal}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-slate-500">Interests</span>
                  <span className="text-sm font-semibold text-slate-900">{interests.length} topics</span>
                </div>
              </div>
              ...
              <div className="flex gap-4">
                <button
                  ...
                  onClick={prevStep}
                  className="flex-1 flex items-center justify-center gap-2 border border-slate-200 hover:bg-slate-50 text-slate-600 font-semibold py-3 rounded-xl transition-all disabled:opacity-50"
                >
                  Back
                </button>
                <button
