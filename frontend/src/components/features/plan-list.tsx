  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-extrabold text-slate-900">{t('myPaths')}</h1>
        <Link 
          href="/plan?view=new"
      ...
      <div className="grid gap-4">
        {plans.length === 0 ? (
          <div className="bg-white p-12 rounded-2xl border-2 border-dashed border-slate-200 text-center">
            ...
            <h3 className="text-lg font-bold text-slate-900">{t('noPaths')}</h3>
            <p className="text-slate-500 mb-6 max-w-xs mx-auto">{t('startGenerating')}</p>
            <Link 
              href="/plan?view=new"
